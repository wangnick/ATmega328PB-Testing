...
class SPIClass {
public:
  SPIClass(uint8_t ss_, uint8_t miso_, uint8_t mosi_, uint8_t sck_, 
      volatile uint8_t* const spcr_, volatile uint8_t* const spsr_, volatile uint8_t* const spdr_)
      : ss(ss_), miso(miso_), mosi(mosi_), sck(sck_), spcr(spcr_), spsr(spsr_), spdr(spdr_)
      , initialized(0), interruptMode(0), interruptMask(0), interruptSave(0)
...
private:
  const uint8_t ss, miso, mosi, sck;
